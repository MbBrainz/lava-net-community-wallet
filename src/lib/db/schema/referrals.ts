...
// ============================================
// TABLE: pending_referral_visits
// ============================================

/**
 * Stores pending referral visits for server-side probabilistic matching.
 *
 * This enables referral attribution even when localStorage is not shared
 * between the browser and PWA (common on iOS).
 *
 * How it works:
 * 1. User visits site with ?ref=CODE - fingerprint stored here
 * 2. User installs PWA (localStorage may be empty)
 * 3. PWA calls match-visit API with current fingerprint
 * 4. Server matches based on IP + User Agent + time window
 * 5. If match found, referral code is returned and record deleted
 *
 * Records are automatically expired after 60 minutes to prevent stale matches.
 */
export const pendingReferralVisits = pgTable("pending_referral_visits", {
  /** Unique identifier */
  id: uuid("id").defaultRandom().primaryKey(),

  /** Visitor's IP address (for matching) */
  ipAddress: varchar("ip_address", { length: 45 }).notNull(), // IPv6 max length

  /** Visitor's User Agent string (for matching) */
  userAgent: varchar("user_agent", { length: 512 }).notNull(),

  /** Screen resolution (optional, for extra matching precision) */
  screenResolution: varchar("screen_resolution", { length: 20 }),

  /** The referral code from URL */
  ...
  /** When this record expires (default: 60 minutes after creation) */
